...
class FDS(nn.Module):
    ...
    def update_running_stats(self, features, labels, epoch):
        ...
        if self.feature_dim != features.size(1):
            print("self.feature_dim:", self.feature_dim, "/ features.size(1):", features.size(1))
        if features.size(0) != labels.size(0):
            print("features.size(0):", features.size(0), "/ labels.size(0):", labels.size(0))
